...
import { isValidNumber } from "./ts/helpers";
import {
    Department,
    Restaurant,
    Filters,
...
// Основные клавиатуры
const mainKeyboard = Markup.keyboard([["/comments"], ["/filters"]]).resize();
...
async function nextStep(ctx: any) {
    ...
    const step = commentsFlowStep.get(chatId) ?? Step.Department;
    try {
        switch (step) {
            ...
            case Step.Dates:
                return sendSkipCancel(
                    ctx,
                    "Шаг 2: Пропустите шаг или введите даты отзывов в формате YYYY-MM-DD:YYYY-MM-DD",
                    skipBtn,
                    cancelBtn
                );
            case Step.Stars:
                return sendSkipCancel(
                    ctx,
                    "Шаг 3: Пропустите шаг или введите количество звезд, поставленных пользователем, одним числом - 1",
                    skipBtn,
                    cancelBtn
                );
            /*
            ...
            case Step.PageSize:
                return sendSkipCancel(
                    ctx,
                    "Шаг 5: Введите количество выводимых отзывов на страницу",
                    skipBtn,
                    cancelBtn
                );
            default:
...
async function fetchAndSend(ctx: any, page: number) {
    const chatId = String(ctx.chat.id);
    try {
        await ctx.reply("Получаем данные, подождите...");
        ...
        await ctx.reply("Не удалось получить комментарии. Попробуйте позже.");
    }
}
...
bot.command("comments", async (ctx) => {
    ...
    commentsFlowStep.set(chatId, Step.Department);
    return nextStep(ctx);
});
...
bot.command("filters", async (ctx) => {
    ...
    commentsFlowStep.set(chatId, Step.Department);
    return sendInlineKeyboard(ctx, "Изменить фильтры?", [
...
bot.on("text", async (ctx) => {
    const chatId = String(ctx.chat.id);
    const step = commentsFlowStep.get(chatId);
    if (step === undefined || step === Step.Department) return;
    const f = getOrInitFilters(chatId);
    ...
    if (step === Step.PageSize && /^[1-9]\d*$/.test(text)) {
        f.page_size = text;
    }
    commentsFlowStep.set(chatId, step + 1);
    return nextStep(ctx);
    ...
    for (const [chatId, f] of userFilters.entries()) {
        try {
            const params: Record<string, string> = {
                ...
                page: "1",
                page_size: f.page_size,
                created_at_after: f.lastChecked,
            };
            const cont = await fetchJSON<CommentContainer>(buildCommentQuery(params));
            for (const c of cont.results) {
